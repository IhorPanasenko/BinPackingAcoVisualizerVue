...
export class AsRankAlgorithm {
   ...
   */
  public initialize() {
    this.heuristics = Array(this.maxItemId).fill(0).map(() => Array(this.maxItemId).fill(0));
    
    // AS-Rank не потребує жадібного запуску.
    // Ми ініціалізуємо феромони базовим значенням tau_0.
    const tau_0 = 1.0; // Початковий феромон
    const newPheromones = Array(this.maxItemId).fill(0).map(() => 
      Array(this.maxItemId).fill(tau_0)
    );
  ...
  public runIteration(): { global: Solution | null, iteration: Solution } {
    ...
    if (!this.globalBestSolution || iterationBestSolution.numBins < this.globalBestSolution.numBins) {
  ...
  private _constructSolution(): Item[] {
    ...
    let currentItem = this.items.find(i => i.id === currentItemId)!;
